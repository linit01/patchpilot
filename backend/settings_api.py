...
# ============================================================================
# Windows Agent Setup Endpoint (no auth — serves public key only)
# ============================================================================

from fastapi import Response

# Separate router without auth for public endpoints
public_router = APIRouter(prefix="/api/settings", tags=["setup"])

@public_router.get("/windows-agent", include_in_schema=False)
async def windows_agent_script(pool: asyncpg.Pool = Depends(get_db_pool)):
    """
    Serve a PowerShell bootstrap script that downloads and runs
    Enable-PatchPilotSSH.ps1 with the default SSH public key pre-injected.
    
    No authentication required — only the public key is exposed.
    
    Usage on Windows (Admin PowerShell):
        powershell -ExecutionPolicy Bypass -Command "irm http://<patchpilot>/api/settings/windows-agent | iex"
    """
    ...
    try:
        ...
        # Derive public key from private key using paramiko
        key_obj = None
    ...
    # 2. Get APP_BASE_URL for the script download URL
    try:
        async with pool.acquire() as conn:
            url_row = await conn.fetchrow(
                "SELECT value FROM settings WHERE key = 'app_base_url'"
            )
        base_url = (url_row['value'].strip().rstrip('/') if url_row and url_row['value'] else None) \
                    or os.getenv('APP_BASE_URL', '').strip().rstrip('/') \
                    or 'http://localhost:8080'
    except Exception:
        base_url = os.getenv('APP_BASE_URL', 'http://localhost:8080').strip().rstrip('/')

    # 3. Read the PS script from disk
    script_paths = [
        '/scripts/windows/Enable-PatchPilotSSH.ps1',
        '/ansible/scripts/windows/Enable-PatchPilotSSH.ps1',
        '/ansible-src/scripts/windows/Enable-PatchPilotSSH.ps1',
        os.path.join(os.path.dirname(__file__), '..', 'scripts', 'windows', 'Enable-PatchPilotSSH.ps1'),
    ]
    ps_script = None
    ...
    if not ps_script:
        # Fallback: return a bootstrap snippet that downloads from the server
        # This handles cases where the script isn't on disk
        bootstrap = f'''# PatchPilot Windows Agent Setup
# Run this in an Administrator PowerShell session

$ErrorActionPreference = 'Stop'
Write-Host "PatchPilot Windows Agent Setup" -ForegroundColor Cyan
Write-Host "Downloading setup script from {base_url}..." -ForegroundColor Yellow

$scriptUrl = "{base_url}/api/settings/windows-agent/script"
$scriptPath = "$env:TEMP\\Enable-PatchPilotSSH.ps1"
Invoke-WebRequest -Uri $scriptUrl -OutFile $scriptPath -UseBasicParsing

& $scriptPath -PublicKey "{pub_key}" -Unattended
Remove-Item $scriptPath -Force -ErrorAction SilentlyContinue
'''
        return Response(content=bootstrap, media_type="text/plain; charset=utf-8")

    # 4. Return a wrapper that writes the full script to temp and runs it with the public key
    wrapper = f'''# PatchPilot Windows Agent Setup
# Run this in an Administrator PowerShell session
...
Write-Host "  PatchPilot -- Windows Agent Installer" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "SSH Public Key: $($pubKey.Substring(0, 40))..." -ForegroundColor Gray
Write-Host ""
...
# Run with the public key pre-injected
& $scriptPath -PublicKey $pubKey -Unattended
