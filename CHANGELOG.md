...
### Changed
- **Default `imagePullPolicy` flipped from `Always` to `IfNotPresent`.** PatchPilot's deployment uses immutable versioned image tags (`backend-1.x.x`, `frontend-1.x.x`), so `Always` provided no benefit and two real downsides: every pod restart re-pulled identical layers from Docker Hub (wasted bandwidth, slower restarts), and any pod restart after an old tag had been deleted from the registry would land in `ImagePullBackOff` even though k3s had the image cached locally. `IfNotPresent` uses the local cache for known tags and only pulls when the tag actually changes (which is exactly what PatchPilot's in-app self-update does — it sets a new tag, k3s pulls the new tag, done). Operators chasing a mutable `:latest`-style tag can override via `patchpilot.image.pullPolicy: Always` in install-config.yaml. Affected files: `install-k3s.sh` default, `install-config.yaml.example` documented default, `webinstall/server.py` Pydantic default, `k8s/deployment.yaml` (3 occurrences in the rendered manifest)
